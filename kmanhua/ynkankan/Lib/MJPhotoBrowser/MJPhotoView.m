    ...
    if (_photo.firstShow) { // 首次显示
        ...
        else
        {
            if (![_photo.url.absoluteString hasSuffix:@"gif"]) {
                [CustomProgressHUD showHUD:self];
                ...
                [_imageView setImageWithURL:_photo.url placeholderImage:_photo.placeholder options:SDWebImageRetryFailed|SDWebImageLowPriority completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType)
                 {
                     ...
                     [CustomProgressHUD hideHUD:photoView];
                 }];
        }
...
- (void)photoStartLoad
{
    if (_photo.image) {
        ...
        _imageView.image = _photo.image;
        [CustomProgressHUD hideHUD:self];
    } else {
...
- (void)adjustFrame
{
    ...
    if (_photo.firstShow) { // 第一次显示的图片
        ...
        _imageView.alpha = 0;
        //_imageView.frame = CGRectMake(0, Screen_Height, imageFrame.size.width, imageFrame.size.height);
...
- (void)hide
{
    ...
    [UIView animateWithDuration:0.3 animations:^{
       // _imageView.frame = [_photo.srcImageView convertRect:_photo.srcImageView.bounds toView:nil];
        ...
        //_imageView.frame = CGRectMake(0, Screen_Height, _imageView.frame.size.width, _imageView.frame.size.height);
